...
class PackageManager:
    ...
    def get_importable_configs(self) -> List[Tuple[str, bool]]:
        ...
        # NOTE: This are the packages that are ENABLED, might add support to show
        # which packages are imported and enabled or imported and disabled
        ...
        for package in available_packages - seen_in_config:
            configs.append((package, True))

        return sorted(configs, key=lambda x: x[0])

    def import_config(self, config_name: str) -> None:
        """Import a configuration from $HOME/.config to packages directory."""
        config_src = path.join(self.home, ".config", config_name)
        ...
        shutil.move(config_src, path.join(config_dest, config_name))
        print(f"{COLORS['GREEN']}✓{COLORS['RESET']} Imported: {config_name}")

    def deimport_config(self, config_name: str) -> None:
        """Restore a configuration from packages directory to $HOME/.config."""
        config_src = path.join(
            self.packages_dir, config_name, ".config", config_name)
